...
*
*	12.03.18	yui-konnu	V1.9	Support for IDA 7. Added SPRG descriptions to auto-comments.
*
***************************************************************************************************/

#define	PPCALTIVEC_VERSION	"V1.9"
...
int	PluginAnalyse(insn_t * insn)
{
	// Get the 
	int	codeBytes = get_dword(insn->ea);
		...
		(opBytes == OP(61)))
	{
		...
		for (int opcodeLoop = 0; opcodeLoop < opcodeArraySize; opcodeLoop++)
		{
			...
			if ((codeBytes & pCurrentOpcode->mask) == pCurrentOpcode->opcode)
			{
				...
				{
					op_t*				operandData = &insn->ops[operandLoop];  //cmd.Operands[operandLoop];
...
static ssize_t idaapi PluginExtensionCallback(void * /*user_data*/, int event_id, va_list va)
{
	...
	case processor_t::ev_out_insn:
	{
		...
		{
			// Output mnemonic
			ctx->out_custom_mnem(g_altivecOpcodes[ctx->insn.itype - altivec_lvebx].name, 10);

			// Output operands
			
			if (ctx->insn.ops[0].shown() && ctx->insn.ops[0].type != o_void)
			{
				ctx->out_one_operand(0);
			}

			if (ctx->insn.ops[1].shown() && ctx->insn.ops[1].type != o_void)
			{
			...
			if (ctx->insn.ops[2].shown() && ctx->insn.ops[2].type != o_void)
			{
				if (ctx->insn.ops[0].shown() || ctx->insn.ops[1].shown())
				{
					ctx->out_symbol(',');
					ctx->out_char(' ');
				}
			...
			if (ctx->insn.ops[3].shown() && ctx->insn.ops[3].type != o_void)
			{
				if (ctx->insn.ops[0].shown() || ctx->insn.ops[1].shown() || ctx->insn.ops[2].shown())
				{
					ctx->out_symbol(',');
					ctx->out_char(' ');
				}
			...
			if (ctx->insn.ops[4].shown() && ctx->insn.ops[4].type != o_void)
			{
				if (ctx->insn.ops[0].shown() || ctx->insn.ops[1].shown() || ctx->insn.ops[2].shown() || ctx->insn.ops[3].shown())
				{
					ctx->out_symbol(',');
					ctx->out_char(' ');
				}
          ...
          (get_cmt(NULL, ctx->insn.ea, true) == -1))
			{
             ...
             indentLoop++)
				...
				for (int i = 0; i < sizeof(ctx->insn.ops) / sizeof(op_t); i++) {
					op_t op = ctx->insn.ops[i];

					if ((op.type == o_reg) && (op.specflag1 & 0x04)) {
						int	sprgArraySize = sizeof(g_cbeaSprgs) / sizeof(cbea_sprg);
						cbea_sprg*	pCurrentSprg = g_cbeaSprgs;
						...
						for (int sprgLoop = 0; sprgLoop < sprgArraySize; sprgLoop++)
						{
							if (op.reg == g_cbeaSprgs[sprgLoop].sprg)
							{
								ctx->out_line(g_cbeaSprgs[sprgLoop].comment, COLOR_AUTOCMT);
							}
